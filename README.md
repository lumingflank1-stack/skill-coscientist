...
Project: LYSMD2 in multiple sclerosis pDC
Observation: LYSMD2 is upregulated in MS pDCs and positively correlated with YBX1, HLA-B, and HLA-C. Proxy KO suggests YBX1 may be a downstream candidate.
...
Project: LYSMD2 in multiple sclerosis pDC
Observation: MS pDC 中 LYSMD2 上调，LYSMD2 与 YBX1、HLA-B、HLA-C 正相关，proxy KO 提示 YBX1 可能是下游候选。
...
I have single-cell data at /path/to/data.rds. First inspect the data structure, then design and run donor-level pseudobulk, module score, partial correlation, and visualization analyses around the LYSMD2-YBX1-pDC axis. After analysis, revise the original hypotheses based on the results.
```
...
我有单细胞数据在 /path/to/data.rds。请先检查数据结构，然后围绕 LYSMD2-YBX1-pDC 轴设计并执行 donor-level pseudobulk、module score、partial correlation 和可视化分析。分析完成后，根据结果修正原始假说。
```
